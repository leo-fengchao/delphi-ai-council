...
### Phase 5 · 完整议会（阶段二 + 三）
- [ ] 引入 XState 重构编排为声明式状态机（阶段增多，ADR-0011 约定的迁移时机；届时另立 ADR）
- [ ] 匿名化 + 评审 prompt 注入与抽取
- [ ] 主席综合 + 固定格式输出解析
- [ ] 共识卡片 / 争议区 / 置信度 UI
- [ ] 单点降级策略
...
### Backlog（非阻塞，择期实现）
- [ ] **更省屏的渲染保活**（优化 ADR-0010）：当前「每站一窗平铺」可靠但占屏。探索：最小化但保持渲染 / 离屏渲染 / 「后台生成 + 仅在抽取瞬间短暂激活」的混合策略，在可靠性与占屏间取得更好平衡。
...
| Q3 | **主席模型固定一家，还是用户可选？** | 影响 Council Page 配置 UI 与编排选主席逻辑 |
| Q4 | **评审阶段是全员互评，还是抽样几家做评审员？** | 影响 DOM 注入次数、耗时与脆弱面 |
| ~~Q5~~ | ✅ **已定稿（2026-06-02）：GitHub raw + 三级回退**，可配置。见 [ADR-0008](docs/adr/0008-config-loader-github-raw.md)。 | — |
| Q6 | **置信度/共识算法**：纯靠主席模型自评输出，还是插件侧再做文本相似度辅助校验？ | 影响阶段三解析与 UI 数据来源 |
